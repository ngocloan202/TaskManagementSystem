...
if ($_SERVER["REQUEST_METHOD"] == "POST" || $_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  if ($ownerResult["isOwner"] == 0) {
    $errorMsg =
      "Bạn không có quyền xóa thành viên! Chỉ người sở hữu dự án mới có thể thực hiện thao tác này.";
    header("Content-Type: application/json");
    echo json_encode(["status" => "error", "message" => $errorMsg]);
    exit();
  }
  ...
  if ($targetUser["UserID"] == $currentUserID) {
    $errorMsg =
      "Bạn không thể tự xóa mình khỏi dự án! Hãy liên hệ người sở hữu khác để được hỗ trợ.";
    header("Content-Type: application/json");
    echo json_encode(["status" => "error", "message" => $errorMsg]);
    exit();
  }
